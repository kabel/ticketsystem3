...
class phpCAS
{
	 ...
	 */
	static function setHTMLFooter($footer)
		{
		global $PHPCAS_CLIENT;
		if ( !is_object($PHPCAS_CLIENT) ) {
			phpCAS::error('this method should not be called before '.__CLASS__.'::client() or '.__CLASS__.'::proxy()');
	 ...
	 */
	static function serviceMail($url,$service,$flags,&$err_code,&$err_msg,&$pt)
		{
		global $PHPCAS_CLIENT, $PHPCAS_AUTH_CHECK_CALL;
	 ...
	 * @param $n an integer.
	 */
	static function setCacheTimesForAuthRecheck($n)
		{
		global $PHPCAS_CLIENT;
		if ( !is_object($PHPCAS_CLIENT) ) {
			phpCAS::error('this method should not be called before '.__CLASS__.'::client() or '.__CLASS__.'::proxy()');
	 ...
	 **/
	static function renewAuthentication() {
		global $PHPCAS_CLIENT, $PHPCAS_AUTH_CHECK_CALL;
     ...
     */
    static function handleLogoutRequests($check_client=true, $allowed_clients=false)
        {
            global $PHPCAS_CLIENT;
            if ( !is_object($PHPCAS_CLIENT) ) {
                phpCAS::error('this method should not be called before '.__CLASS__.'::client() or '.__CLASS__.'::proxy()');
	 ...
	 */
	static function logoutWithRedirectService($service) {
		global $PHPCAS_CLIENT;
		phpCAS::traceBegin();
		if ( !is_object($PHPCAS_CLIENT) ) {
			phpCAS::error('this method should only be called after '.__CLASS__.'::client() or'.__CLASS__.'::proxy()');
	 ...
	 */
	static function logoutWithUrl($url) {
		global $PHPCAS_CLIENT;
		phpCAS::traceBegin();
		if ( !is_object($PHPCAS_CLIENT) ) {
			phpCAS::error('this method should only be called after '.__CLASS__.'::client() or'.__CLASS__.'::proxy()');
	 ...
	 * @param $url a URL that will be transmitted to the CAS server
	 */
	static function logoutWithRedirectServiceAndUrl($service, $url) {
		global $PHPCAS_CLIENT;
		phpCAS::traceBegin();
		if ( !is_object($PHPCAS_CLIENT) ) {
			phpCAS::error('this method should only be called after '.__CLASS__.'::client() or'.__CLASS__.'::proxy()');
	 ...
	 */
	static function setCasServerCert($cert)
		{
		global $PHPCAS_CLIENT;
		phpCAS::traceBegin();
		if ( !is_object($PHPCAS_CLIENT) ) {
	 ...
	 */
	static function setCasServerCACert($cert)
		{
		global $PHPCAS_CLIENT;
		phpCAS::traceBegin();
		if ( !is_object($PHPCAS_CLIENT) ) {
   ...
   */
   static function setExtraCurlOption($key, $value)
		{
		  global $PHPCAS_CLIENT;
		  phpCAS::traceBegin();
		  if ( !is_object($PHPCAS_CLIENT) ) {
